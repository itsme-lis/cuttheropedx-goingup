...
namespace CutTheRopeDX.GameMain
{
    internal sealed partial class GameScene
    {
        ...
        public GameScene()
        {
            ...
            ApplyBackgroundTheme(cTRRootController);
            for (int i = 0; i < 3; i++)
            {
        ...
        /// <summary>
        /// (Re)builds the scrolling background tile from the current level's effective theme
        /// (its own <c>themeBox</c> override if set, otherwise the active pack). Recreates
        /// <see cref="back"/> as a fresh <see cref="TileMap"/> rather than reassigning its texture,
        /// since re-adding a tile quad to an existing TileMap stacks a new drawer instead of
        /// replacing the old one. Called once from the constructor as a safe default, and again
        /// from <see cref="LoadAllLevelMetadata"/> every time a level's metadata loads — the point
        /// at which the level's own theme override is already known to be resolved.
        /// </summary>
        /// <param name="rc">Shared root controller.</param>
        private void ApplyBackgroundTheme(CTRRootController rc)
        {
            string[] boxBackgrounds = PackConfig.GetBoxBackgrounds(rc.GetEffectiveThemePack());
            ...
            {
                throw new InvalidDataException($"Pack config is missing boxBackground for pack {rc.GetEffectiveThemePack()}.");
            }
        ...
        public void CreateEarthImageWithOffsetXY(float xs, float ys)
        {
            ...
            CTRRootController cTRRootController = (CTRRootController)Application.SharedRootController();
            Vector? earthBgPosition = PackConfig.GetEarthBgPosition(cTRRootController.GetEffectiveThemePack());
