...
namespace CutTheRopeDX.GameMain
{
    ...
    internal sealed class CTRRootController : RootController
    {
        ...
        public void PrepareMapAndEnsureResources(XElement map, string newMapName)
        {
            ...
            LoadEffectiveThemeBackground(resourceMgr);
        ...
        /// <summary>Sets the current level's visual-theme override (0-based pack index), or null to clear it.</summary>
        /// <param name="packIndex">Pack index whose visuals should render for this level, or <see langword="null"/>.</param>
        public void SetLevelThemeOverride(int? packIndex)
        {
            _levelThemeOverride = packIndex;
        }

        /// <summary>
        /// Gets the pack index whose visuals (background, sitting platform, ghost-grab color, etc.)
        /// should render for the currently loaded level.
        /// </summary>
        /// <returns>The level's own theme override if set, otherwise the active pack.</returns>
        public int GetEffectiveThemePack()
        {
            return _levelThemeOverride ?? GetPack();
        }

        /// <summary>
        /// Scans the given level XML for an optional <c>themeBox</c> attribute on its
        /// <c>gameDesign</c> element (1-based box number) and sets it as the level's theme override.
        /// Called from <see cref="PrepareMapAndEnsureResources"/> — the single choke point every
        /// level load passes through, including picker entry, "next level," and restarts — so this
        /// always runs before any resource load or visual lookup that depends on the effective theme.
        /// </summary>
        /// <param name="mapOverride">The map XML to scan; falls back to <see cref="loadedMap"/> when omitted.</param>
        internal void DetectLevelThemeOverride(XElement mapOverride = null)
        {
            string themeBoxAttr = (mapOverride ?? loadedMap)?
                .Descendants("gameDesign")
                .FirstOrDefault()?
                .Attribute("themeBox")?
                .Value;

            SetLevelThemeOverride(
                int.TryParse(themeBoxAttr, out int themeBoxNumber) && themeBoxNumber > 0
                    ? themeBoxNumber - 1
                    : null);
        }

        /// <summary>
        /// Loads the current level's theme-override background (if different from the active pack)
        /// and tracks it so it gets freed on the next return to the menu.
        /// </summary>
        /// <param name="resourceMgr">Shared resource manager.</param>
        internal void LoadEffectiveThemeBackground(CTRResourceMgr resourceMgr)
        {
            int effectiveTheme = GetEffectiveThemePack();
            if (effectiveTheme == pack)
            {
                // Already covered by the active pack's own background load/free elsewhere;
                // tracking it here too would cause a double free on menu exit.
                return;
            }

            resourceMgr.LoadPack(PackConfig.GetBoxBackgrounds(effectiveTheme));
                ...
                case 3:
                    {
                        ...
                        if (exitCode <= 2)
                        {
                            ...
                            foreach (int themeIndex in _loadedThemePacks)
                            {
                                resourceMgr.FreePack(PackConfig.GetBoxBackgrounds(themeIndex));
                            }
        ...
        /// <summary>Per-level theme override (0-based pack index) set from the level's <c>themeBox</c> XML attribute.</summary>
        private int? _levelThemeOverride;
