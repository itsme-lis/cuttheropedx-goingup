...
namespace CutTheRopeDX.GameMain
{
    ...
    internal sealed partial class GameScene
    {
        ...
        private void LoadAllLevelMetadata(XElement mapNode, float scale, float offsetY, out float offsetX, out int mapOffsetX, out int mapOffsetY)
        {
            ...
            // Detect this level's theme override fresh from the map we were actually given, rather
            // than relying on an earlier step (like PrepareMapAndEnsureResources) having already run
            // for this specific map. Show() calls this directly with GetMap() for a freshly-entered
            // level, bypassing that earlier step entirely - so detection has to be self-contained here.
            rc.DetectLevelThemeOverride(mapNode);
            // Re-apply the background theme now that the override (if any) is known, and make sure
            // its texture is actually loaded - PrepareMapAndEnsureResources only loads it when this
            // level was reached via LoadNextMap; a level entered directly from the picker never went
            // through that path, so it has to be ensured here instead.
            rc.LoadEffectiveThemeBackground(Application.SharedResourceMgr());
            ApplyBackgroundTheme(rc);
                ...
                foreach (XElement item2 in xmlnode.Elements())
                {
                    switch (item2.Name.LocalName)
                    {
                        case "map":
                            ...
                            if (PackConfig.GetEarthBg(rc.GetEffectiveThemePack()))
                            {
